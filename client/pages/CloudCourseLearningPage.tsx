...
const CloudCourseLearningPage: React.FC = () => {
...
//visit this section
  ...
  getDoc(userRef).then((snap) => {
    if (!snap.exists() || snap.data().progress_cc?.roadmap !== 'cloud') return;
    const progress = snap.exists() ? snap.data().progress_cc || {} : {};
    const current = progress[lvlKey] || 0;
    if (current < maxCounts[lvlKey]) {
      updateDoc(userRef, {
        [`progress_cc.${lvlKey}`]: increment(1),
      })
      .then(() => {
      ...
      .catch(async (error) => {
        if (error.code === 'failed-precondition') {
          try {
            ...
            if (!userSnap.exists() || userSnap.data().progress_cc?.roadmap !== 'cloud') return;
            ...
            if (currentCount >= maxCounts[lvlKey]) {
              await updateDoc(userRef, {
                [`progress_cc.${lvlKey}`]: maxCounts[lvlKey]
              });
              setProgressCounts(prev => ({
                ...prev,
...
useEffect(() => {
  const fetchProgress = async () => {
    ...
    try {
      ...
      const progress = userSnap.data()?.progress_cc || {};
  ...
  fetch("http://localhost:8000/api/cloud-course")
    .then((res) => res.json())
    .then((data) => {
      setDocs(data);
      setLoading(false);
  ...
  const fetchQuizQuestions = async (level: string) => {
    setLoadingQuiz(true);
    try {
      const response = await fetch('http://localhost:8000/api/quiz/cloud/generate-questions', {
        method: 'POST',
        headers: {
  ...
  if (lvl === "basic") {
    updates["progress_cc.basic_test_score"] = score;
    // updates["progress.basic_certificate_received"] = pass ? 1 : 0;
  } else if (lvl === "intermediate") {
    updates["progress_cc.intermediate_test_score"] = score;
    ...
    updates["progress_cc.advanced_test_score"] = score;
    // updates["progress.advanced_certificate_received"] = pass ? 1 : 0;
  }

  try {
    const userSnap = await getDoc(userRef);
     if (!userSnap.exists() || userSnap.data().progress_ml?.roadmap !== 'cloud') return;
  ...
  const getGroupedDocs = () => {
    const grouped = docs.reduce((acc, doc, index) => {
      ...
      return acc;
    }, {} as Record<string, { doc: CloudDoc; originalIndex: number }[]>);
    return grouped;
  };
        ...
        <div className="bg-gradient-to-r from-sidebar-primary to-sidebar-primary/90 text-sidebar-primary-foreground p-6 relative overflow-hidden flex-shrink-0">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className={`flex items-center gap-3 ${sidebarCollapsed ? 'justify-center' : ''}`}>
              ...
              {!sidebarCollapsed && (
                <div>
                  <h1 className="text-xl font-bold">Cloud Computing</h1>
                  ...
                  <div className="text-center py-8">
                    ...
                    {(() => {
                      ...
                      if (isFinalLevel && passedTest) {
                        return (
                          <a
                            href={`/certificate/${encodeURIComponent("Cloud Computing Fundamentals")}`}
                            target="_blank"
...
export default CloudCourseLearningPage;